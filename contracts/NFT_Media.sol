...
contract BasicCreate is ERC721Holder {
    ...
    function create_user_table() public payable {
     ...
     function update_like(uint256 like_count,uint256 id)external payable{
        _tableland.runSQL(
            address(this),
            tableId,string.concat("UPDATE ",tableName," SET like = ",Strings.toString(like_count),"WHERE id = ",Strings.toString(id))

        );
    }

    function update_dislike(uint256 dislike_count,uint256 id)external payable{
        _tableland.runSQL(
            address(this),
            tableId,string.concat("UPDATE ",tableName," SET dislike = ",Strings.toString(dislike_count),"WHERE id = ",Strings.toString(id))
        ...
        _tableland.runSQL(
            address(this),
            tableId,string.concat("UPDATE ",tableName," SET like = ",Strings.toString(share_count),"WHERE id = ",Strings.toString(id))

        );
    }

    function insert(uint256 id, string memory comments,uint256 views,uint256 share,uint256 follower,uint256 like,uint256 dislike,string memory time,string memory date) public payable {
        _tableland.runSQL(
            address(this),
            tableId,string.concat("INSERT INTO ",tableName," (id, address,comments,views,share,follower,like,dislike,time,date) VALUES ( ",Strings.toString(id),", '",Strings.toHexString(uint160(msg.sender)),"' , '",comments,"' , ",Strings.toString(views),", ",Strings.toString(share),", ",Strings.toString(follower),", ",Strings.toString(like),", ",Strings.toString(dislike),",'",time,"','",date,"' )")
        );
    }
    ...
    function add_user(address new_user)public{
        ...
        for (uint i = 0; i < users.length; i++) {
            if (users[i] != new_user) {
                ...
                insert(1,"testing_point",posts_count,4,5*800,6,5,buyer," ");
            }
        }
    }
